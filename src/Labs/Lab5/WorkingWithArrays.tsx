...
<input defaultValue={todo.id} className="form-control w-50" onChange={(e) => setTodo({ ...todo, id: e.target.value })}/><hr/>
  ...
  <h3>Updating an Item in an Array</h3>
    ...
    <h3>Updating Todo Completed Status in an Array</h3>
      <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
        Update Completed Status</a>
      <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
      <input type="checkbox" className="form-check-input me-2" id="wd-update-completed-by-id"
      ...
      <input id="wd-input-update-description" defaultValue={todo.id} className="form-control w-25 float-start me-2"
